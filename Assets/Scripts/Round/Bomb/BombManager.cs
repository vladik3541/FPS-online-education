...
public class BombManager : MonoBehaviourPunCallbacks
{
    ...
    public void UpdatePlantingProgress()
    {
        ...
        // Debug для перевірки
        Debug.Log($"Plant Progress: {elapsed:F2}/{plantDuration} секунд");
        
        if (elapsed >= plantDuration)
        {
            // ✅ ВИПРАВЛЕННЯ: Викликаємо RPC навіть якщо не MasterClient
            // MasterClient перевірить і виконає
            photonView.RPC(nameof(PlantBomb_RPC), RpcTarget.MasterClient, bombSites[plantingSiteIndex].position, PhotonNetwork.Time, currentPlantingPlayer);
        }
    }
    ...
    {
        // ✅ ВИПРАВЛЕННЯ: Тільки MasterClient виконує логіку один раз
        if (!PhotonNetwork.IsMasterClient) return;
        
        // Синхронізуємо стан для всіх
        photonView.RPC(nameof(OnBombPlanted_RPC), RpcTarget.All, position, plantTime, planterActorId);
    }
    
    [PunRPC]
    private void OnBombPlanted_RPC(Vector3 position, double plantTime, int planterActorId)
    {
    ...
    public void UpdateDefusingProgress()
    {
        ...
        // Debug для перевірки
        Debug.Log($"Defuse Progress: {elapsed:F2}/{duration} секунд");
        
        if (elapsed >= duration)
        {
            // ✅ ВИПРАВЛЕННЯ: Викликаємо RPC навіть якщо не MasterClient
            photonView.RPC(nameof(DefuseBomb_RPC), RpcTarget.MasterClient, currentDefusingPlayer);
        }
    }
